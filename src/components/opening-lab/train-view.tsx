...
export function TrainView({ pack, line, onBack }: Props) {
  ...
  const beginSlide = useCallback(
    (
      ...
      // Board state (fen) stays pre-move until the glide finishes
  ...
  const onSlideComplete = useCallback(() => {
    ...
    pendingCommit.current = null;
    // Commit fen FIRST so the same DOM node lands on the destination square
    // with no empty frame, then clear the slide flag in the same batch.
